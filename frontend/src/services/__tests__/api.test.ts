/**
 * Unit tests for the API service module (src/services/api.ts).
 *
 * Covers:
 *   - healthCheck() - fetches /api/health
 *   - getRoot() - fetches /api/
 *   - getConfig() - fetches /api/config
 *   - sendChat() - POST /api/chat
 *   - streamChat() - POST /api/chat/stream with SSE parsing
...
describe('api service', () => {
  ...
  describe('streamChat()', () => {
    it('POSTs to /api/chat/stream and invokes callbacks for SSE events', async () => {
      const sseData = [
        'event: token\n',
